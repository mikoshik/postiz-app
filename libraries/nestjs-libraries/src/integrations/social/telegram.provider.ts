...
export class TelegramProvider extends SocialAbstract implements SocialProvider {
  ...
  async post(
    ...
    for (const message of postDetails) {
      ...
      // if there are multiple media, bot sends them as a media group - max 10 media per group
      // CUSTOM: Отправляем только первые 10 медиафайлов, остальные игнорируются
      else {
        const mediaGroups = this.chunkMedia(processedMedia, 10);
        // Берём только первую группу (первые 10 медиа)
        const firstGroup = mediaGroups[0];
        const mediaGroup = firstGroup.map((m, index) => ({
